...
window.chatVmDiagnostics = {
  ...
  get cpuRunning() {
    return Boolean(emulator?.cpu_is_running);
  }
};
...
async function waitForScreenText(test, timeoutMs = 70000) {
  ...
  while (performance.now() - startedAt < timeoutMs) {
    ...
    if (typeof test === "string" ? text.includes(test) : test(text)) return text;
...
async function prepareAssets() {
  if (typeof window.V86 !== "function") {
    throw new Error("공식 v86 런타임을 불러오지 못했습니다.");
  }
  setStatus("실행 자산 준비 중…", 5);
  const [hda, robby] = await Promise.all([loadHda(), fetchRobby()]);
  setStatus("Robby Roto ROM 디스크 생성 중…", 72);
  prepared = { hda, hdb: makeRomDisk(robby) };
}

function addVmEvent(name) {
  emulator.add_listener(name, (...args) => {
    eventLog.push({ name, at: Date.now(), args });
    console.log(`[v86] ${name}`, ...args);
  });
}
...
async function bootVm() {
  ...
  emulator = new window.V86({
    ...
    vga_bios: { url: ASSETS.vga },
    fda: { url: ASSETS.freedos },
  ...
  ["download-error", "emulator-loaded", "emulator-ready", "emulator-started", "emulator-stopped"]
    .forEach(addVmEvent);

  emulator.add_listener("emulator-loaded", () => {
    if (generation === bootGeneration) setStatus("VM 로드 완료 · FreeDOS 시작 중", 88);
  });
  emulator.add_listener("emulator-started", () => {
    if (generation === bootGeneration) setStatus("VM 실행 중 · FreeDOS 프롬프트 대기", 92);
  });
  ...
  try {
    ...
    if (!emulator.cpu_is_running) {
      throw new Error(`v86 CPU가 시작되지 않았습니다.\n이벤트: ${JSON.stringify(eventLog)}`);
    }
...
const scanCodes = {
  up: [0xe0, 0x48, 0xe0, 0xc8], down: [0xe0, 0x50, 0xe0, 0xd0],
  left: [0xe0, 0x4b, 0xe0, 0xcb], right: [0xe0, 0x4d, 0xe0, 0xcd],
  enter: [0x1c, 0x9c], esc: [0x01, 0x81], ctrl: [0x1d, 0x9d],
  alt: [0x38, 0xb8], space: [0x39, 0xb9], "1": [0x02, 0x82],
  "5": [0x06, 0x86], o: [0x18, 0x98], k: [0x25, 0xa5]
};
